...
def warna():
    return [color.blue, color.yellow, color.red, color.green]
...
green_ball = sphere(pos=vector(random.uniform(-5, 5), random.uniform(-5, 5), 0),
                  radius=0.5,
                  color=color.green)
green_ball.velocity = random_velocity()
...
while True:
  ...
  for ball in balls:
    ...
    if mag(ball.pos - green_ball.pos) <= (ball.radius + green_ball.radius):
      # pass
      if ball.color == green_ball.color:
        ball.velocity.x *= -1
        ball.velocity.y *= -1

        if green_ball.velocity.x > ball.velocity.x:
          green_ball.pos.x += green_ball.velocity.x * 0.1
        else:
          green_ball.pos.x -= green_ball.velocity.x * 0.1
        if green_ball.velocity.y > ball.velocity.y:
          green_ball.pos.y += green_ball.velocity.y * 0.1
        else:
          green_ball.pos.y -= green_ball.velocity.y * 0.1
        ...
        #   ball.velocity.x *= -1
        # ball.velocity.y *= -1
        # ball.velocity.x = random.uniform(-1, 1); 
        # ball.velocity.y = random.uniform(-1, 1);
      else:
